...
class DevicelogAction extends Action
{
	protected $url_base = '';
	...
	protected function mlist()
	{
		...
		$deviceid = Zhimin::request('deviceid');
		...
		if ($deviceid != '') {
			$wsql .= ' and  pv1 . deviceid  = \'' . $deviceid . '\'';
		}
		...
		$sql = 'SELECT COUNT(*) as count FROM ' . $log_m->table() . ' as pv1 left join zm_device as pd on pd.hostbody=pv1.deviceid' . "\r\n" . '        left join zm_danwei as pda on pda.bh=pd.danwei where ' . $wsql;
...
*/
		...
		//$sql = 'SELECT `pl`.*,`pd`.`dname` FROM `' . $log_m->table() . '` as `pl` left join `zm_danwei` as `pd` on `pl`.`unit_no`=`pd`.`bh` where `pl`.id in (' . $post_id_string . ') order by id desc ' . $limit;
		...
		$this->exportToExcel('记录仪日志', $log_types, $result, 'log_'.date("YmjHis"));
	}
	...
	function exportToExcel($top_title, $logtypes, $data, $outputFileName) {
		...
		$tableheader = array('序号', '所属单位', '警员姓名（警员编号）', '操作类型', '记录仪编号', '操作时间', '文件名');
		...
		foreach($data as & $val) {
			...
			$objPHPExcel -> getActiveSheet(0) -> setCellValue('C'.($i + 4), $val['hostname'].'('.$val['hostcode'].')');
			$objPHPExcel -> getActiveSheet(0) -> setCellValue('D'.($i + 4), $logtypes[$val['logtype']]);
			$objPHPExcel -> getActiveSheet(0) -> setCellValue('E'.($i + 4), chunk_split($val['deviceid'], 9, " "));
			$objPHPExcel -> getActiveSheet(0) -> setCellValue('F'.($i + 4), $val['logtime']);
			$objPHPExcel -> getActiveSheet(0) -> setCellValue('G'.($i + 4), $val['filename']);
			$objPHPExcel -> getActiveSheet() -> getStyle('A'.($i + 4).':G'.($i + 4)) -> getAlignment() -> setVertical(PHPExcel_Style_Alignment::VERTICAL_CENTER);
			$objPHPExcel -> getActiveSheet() -> getStyle('A'.($i + 4).':G'.($i + 4)) -> getBorders() -> getAllBorders() -> setBorderStyle(PHPExcel_Style_Border::BORDER_THIN);
