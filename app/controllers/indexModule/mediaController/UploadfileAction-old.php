		...
		foreach ($filelist_array as $fa){
			...
			if(!empty($fa)){
				...
				if($filetype == "MP4"){  //通过ffmpeg获取视频（MP4）的播放时长和缩略图
					...
					$ffmpegInstance = new ffmpeg_movie($filepath_tmp);  
					$playtime = $ffmpegInstance->getDuration();

					$ff_frame = $ffmpegInstance->getFrame(2);//截取视频第2帧的图像
					$gd_image = $ff_frame->toGDImage(); 
					$thumb = 'media/' . $dt . '/' . $filetype . '/' . $filename . '.THM.JPG';
					$img = $document_root . $thumb;  //要生成图片的绝对路径
					imagejpeg($gd_image, $img, 100);//创建jpg图像
					$this->simple_thumb($img, 320, 240 , $img);  //生成指定宽高的缩略图并覆盖原图
				}
