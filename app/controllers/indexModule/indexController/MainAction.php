...
class MainAction extends Action
{
	...
	protected function _main()
	{
		...
		$unit_m->get_subs_by_sn($danwei_array, $_SESSION['unitcode'], $deep_flg);
		...
		if ($auth->isSuperAdmin()) {
			...
			//modify：首页顶部（数据、用户、设备、单位）统计
			$sql_tongji = 'SELECT count(id) as data_count from zm_video_list where danwei in (' . $dlist1 . ')';
			$res_tongji = $unit_m->fetchOne('', $sql_tongji);
			$data['data_count'] = $res_tongji['data_count'];

			$sql_tongji = 'SELECT count(id) as user_count from zm_user where dbh in (' . $dlist1 . ')';
			$res_tongji = $unit_m->fetchOne('', $sql_tongji);
			$data['user_count'] = $res_tongji['user_count'];
			
			$sql_tongji = 'SELECT count(id) as device_count from zm_device where danwei in (' . $dlist1 . ')';
			$res_tongji = $unit_m->fetchOne('', $sql_tongji);
			$data['device_count'] = $res_tongji['device_count'];

			$sql_tongji = 'SELECT count(id) as danwei_count from zm_danwei where unitsyscode like \'' . $_SESSION['unitsyscode'] . '%\'';
			$res_tongji = $unit_m->fetchOne('', $sql_tongji);
			$data['danwei_count'] = $res_tongji['danwei_count'];
			//
		}
			...
			//modify：首页顶部（数据、用户、设备、单位）统计
			$sql_tongji = 'SELECT count(id) as data_count from zm_video_list where hostcode = \'' . $_SESSION['hostcode'] . '\'';
			$res_tongji = $unit_m->fetchOne('', $sql_tongji);
			$data['data_count'] = $res_tongji['data_count'];
		...
		$announces_sql = 'select * from `zm_announce` WHERE `enddate`>\'' . time() . '\' and (`receive_unit` in (' . $dlist . ') or `receive_unit`=\'\' or `receive_unit` is NULL) order by `vieworder` asc,`createtime` desc';
