...
class Core_Lib_Page
{
	...
	public function __construct($data = array())
	{
		...
		if (!empty($this->total_pages) && ($this->total_pages < $this->now_page)) {
		...
		if ($this->total_pages != 1) {
			...
			for ($i = 1; $i <= $this->total_pages; $i++) {
				if ($i == $this->now_page) {
					...
					if ((4 <= $this->now_page - $i) && ($i != 1)) {
						$return .= '<span class=\'pageMore\'>...</span>' . "\n" . '';
						$i = $this->now_page - 3;
					}
					else {
						if ((($this->now_page + 5) <= $i) && ($i != $this->total_pages)) {
							$return .= '<span>...</span>' . "\n" . '';
							$i = $this->total_pages;
						}
	...
	protected function show_1()
	{
		...
		if ($this->total_pages < ($plus + $this->now_page)) {
