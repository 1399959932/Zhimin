$(document).ready(function(){
	...
	$(".condition_top .sure_add").click(function(){
		$.ajax({
			url: './?_a=stnotify&_c=system&_m=index&action=add',
			method: 'post',
			data: $('#stnotify_add_form').serialize(),
			dataType: 'json',
   ...
   $(".sure_one_del").click(function(){    	
        ...
        $.ajax({
			url: './?_a=stnotify&_c=system&_m=index&action=del',
			method: 'post',
			data: {id:id},
			dataType: 'json',
			success: function(data) {
	...
	// 查看
		$(".view_a").click(function(){
			...
			var id=$(".tbody_atten tr.tr_on").attr("date");   //得到记录id
			var sendUrl='./?_a=stnotify&_c=system&_m=index&action=view&id='+id;
			$.ajax({
	            type: "POST",
	            url: sendUrl,
	            dataType: "html",
