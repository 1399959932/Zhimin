$(document).ready(function(){
	...
	$(".add_pollcode").click(function(){
		$.ajax({
			url: './?_a=pollcode&_c=system&_m=index&action=create_code',
			method: 'post',
			dataType: 'json',
			success: function(data) {
				if(data.state == 'success'){ //如果成功
					var code = data.msg;
					$('#add_code').html(code);
					$('#code_input').val(code);
					layer.open({
	...
	$(".condition_top #add_sure").click(function(){
		$.ajax({
			url: './?_a=pollcode&_c=system&_m=index&action=add',
			method: 'post',
   ...
   $(".sure_one_del").click(function(){    	
        ...
        $.ajax({
			url: './?_a=pollcode&_c=system&_m=index&action=del',
			method: 'post',
			data: {id:id},
			dataType: 'json',
			success: function(data) {
	...
	$(".edit_a").click(function(){
		if($('.edit_a').hasClass('on')){   //如果点亮，说明可以操作,则出现弹出框
			...
			$.ajax({
	            type: "POST",
	            url: './?_a=pollcode&_c=system&_m=index&action=edit',
	            data: {id:id},
	            dataType: "json",
	            success: function(data){
	            	if(data.state == 'fail'){   //获取数据如果失败
     					...
     					$('#edit_code').html(data.code);
     					$('#edit_expira_time').val(data.expira_time);
